...
def divergence(A):
  nr, nc = A.shape
  row_player = np.matmul(A, np.ones([nc, 1])) / nc
  col_player = np.matmul(np.ones([1, nr]), np.transpose(A)) / nr
  return (row_player, col_player)


def nash_divergence(A):
  re, ce = gnm_solve(A)
  nr, nc = A.shape
  row_player = np.matmul(A, np.reshape(ce, [nc, 1]))
  col_player = np.matmul(np.reshape(re, [1, nr]), np.transpose(A))
  return (row_player, col_player)
...
def grad(A):
  nr, nc = A.shape
  rv, cv = divergence(A)
  return np.matmul(rv, np.ones([1, nc])) - np.matmul(np.ones([nr, 1]),
                                                     np.reshape(cv, [1, nc]))
...
def cyclic_schur_decomp(A):
  G = grad(A)
  C = A - G
  T, Z = scipy.linalg.schur(C)
  return G, T, Z
