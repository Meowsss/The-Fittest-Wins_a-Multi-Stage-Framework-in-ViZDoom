...
class AEMatchMakingGameMgr(GameMgr):
  ...
  def add_player(self, p, parent_p=None, learner_id=None, **kwargs):
    ...
    if learner_id is not None:
      ...
      elif 'EE' in self._lrn_role_list[self._lrn_id_list.index(learner_id)]:
        role_cls = IndivEvoExploiter
        ss = self._lrn_role_list[self._lrn_id_list.index(learner_id)].split('-')
        assert len(ss) == 2, 'EE role name incorrect.'
        pattern = ss[-1]
      else:
  ...
  def get_player(self, current_player):
    ...
    if isinstance(indiv, IndivEvoExploiter):
...
class AEMatchMakingGameMgrV2(AEMatchMakingGameMgr):
  """Agent-Exploiter Match Making Game Manager, V2.

  Lxhan's modification over [7].
  """
  def __init__(self,
               pgn_file=None,
               log_file='aev2.log',
               ...
               **kwargs):
    self._lrn_id_list = lrn_id_list or ['lrngrp0', 'lrngrp1', 'lrngrp2']
    self._lrn_role_list = lrn_role_list or ['MA', 'ME', 'CE']
    super(AEMatchMakingGameMgrV2, self).__init__(
      pgn_file, log_file, stat_decay, lrn_id_list,
      self._lrn_role_list, verbose=0, **kwargs)


class SKMatchMakingGameMgr(AEMatchMakingGameMgr):
  """
  A Si Ke game mgr that the main agent should not generate new player
  ...
  def _add_player_individual(self, p, parent_p, role_cls):
    if role_cls == IndivMainAgent:
      role_cls = IndivSKMainAgent
    super(SKMatchMakingGameMgr, self)._add_player_individual(
      p, parent_p, role_cls)
