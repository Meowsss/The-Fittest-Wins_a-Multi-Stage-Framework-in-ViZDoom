...
def _create_cig2107_track1_env(env_config, inter_config):
  ...
  train_mode = (1 if 'train_mode' not in env_config
                else env_config['train_mode'])  # 1 for train frag, 0 for train navigation
  evaluation = (0 if 'evaluation' not in env_config
                else env_config['evaluation'])
  if evaluation:
    from arena.sandbox.vizdoom_vs_f1 import VizdoomMPEnv, VizdoomVecRwd
  else:
