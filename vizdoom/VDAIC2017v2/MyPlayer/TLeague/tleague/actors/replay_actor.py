...
def _get_interface(map_name, crop=False):
...
class ReplayExtractor(object):
  ...
  def __init__(self, replay_dir, replay_filename, player_id, replay_converter,
               ...
               crop_to_playable_area=False, unk_mmr_dft_to=4000):
  ...
  def _extract(self):
    ...
    with run_config.start(version=self._version) as controller:
      ...
      controller.start_replay(sc_pb.RequestStartReplay(
        ...
        map_data=map_data,
        options=_get_interface(map_name, crop=self._crop_to_playable_area),
  ...
  def run(self):
    ...
    while self.replay_task != "":
      ...
      crop_to_playable_area = (
        False if 'crop_to_playable_area' not in self.converter_config
        else self.converter_config['crop_to_playable_area']
      )
