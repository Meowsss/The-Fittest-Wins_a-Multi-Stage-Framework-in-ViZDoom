...
class PPOLearner(BaseLearner):
  ...
  def __init__(self, league_mgr_addr, model_pool_addrs, learner_ports,
               ...
               unroll_length=32, rollout_length=1, use_mixed_precision=False,
    ...
    self._data_server = DataServer(self._pull_data, rm_size,
                                   ...
                                   prefetch_buffer_size=pull_worker_num)
    ...
    with tf.device('/cpu:0'):
      inputs_ph = policy.net_inputs_placeholders_fun(net_config)
      model = create_policy(inputs_ph, net_config)
    self.params = tf.trainable_variables(scope='model')
    ## Note: Assumed model/vf scope for value head
    ...
    with tf.device('/cpu:0'):
      ...
      grads_and_vars, self.clip_grad_norm, self.nonclip_grad_norm = self.clip_grads_vars(
        list(zip(avg_grads, self.params)), clip_vars, max_grad_norm)
      grads_and_vars_vf, self.clip_grad_norm_vf, self.nonclip_grad_norm_vf = self.clip_grads_vars(
        list(zip(avg_grads_vf, self.params_vf)), clip_vars, max_grad_norm)
  ...
  def _init_const(self, total_timesteps, burn_in_timesteps, batch_size,
    ...
    self.ep_loss_coef = ep_loss_coef
  ...
  def build_loss(self, model, input_data):
    ...
    if isinstance(self.ent_coef, list):
      ...
      entropy = tf.reduce_mean([e*ec for e, ec in zip(entropy_list, self.ent_coef)])
    else:
      entropy = tf.reduce_mean(entropy_list) * self.ent_coef
    ...
    if self.distillation:
      ...
      if isinstance(self.distill_coef, list):
        ...
        distill_loss = tf.reduce_mean([d*dc for d, dc in zip(distill_losses, self.distill_coef)])
      else:
        distill_loss = tf.reduce_mean(distill_losses) * self.distill_coef
    if isinstance(self.vf_coef, list):
      ...
      value_loss = tf.reduce_mean(model.loss.value_loss * tf.constant(self.vf_coef))
    else:
      value_loss = tf.reduce_mean(model.loss.value_loss) * self.vf_coef
    ...
    loss = (model.loss.ppo_loss + value_loss - entropy + distill_loss + ep_loss)
  ...
  def _run_train_loop(self, nbatch):
    ...
    if self._need_burn_in:
      burn_in_timesteps = getattr(self.task.hyperparam, 'burn_in_timesteps', self.burn_in_timesteps)
