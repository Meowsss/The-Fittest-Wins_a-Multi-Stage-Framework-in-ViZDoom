...
from tleague.learners.replay_memory import ReplayMem, ImpSampReplayMem
...
def extract_dtype_shape_from_gym(gym_space):
  if isinstance(gym_space, spaces.Tuple):
    dtype, shape = zip(*[extract_dtype_shape_from_gym(space) for space in gym_space.spaces])
  elif isinstance(gym_space, spaces.Dict):
    keys, sps = zip(*gym_space.spaces.items())
    dtype, shape = zip(*[extract_dtype_shape_from_gym(sp) for sp in sps])
    dtype = OrderedDict(zip(keys, dtype))
    shape = OrderedDict(zip(keys, shape))
  else:
    dtype = tf.as_dtype(gym_space.dtype)
...
class DataServer(object):
  ...
  def __init__(self, data_getter, rm_size, unroll_length, batch_size,
    ...
    self._replay_mem = ReplayMem(rm_size, unroll_length, batch_size, rollout_length)
    ...
    self._infos = deque(maxlen=100)
    self._log_infos_interval = 20
    self._info_num = 0
      ...
      dataset = tf.data.Dataset.range(batch_worker_num).apply(
        tf.contrib.data.parallel_interleave(
          lambda x: tf.data.Dataset.from_generator(
            self._data_generator, self.dtypes, self.shapes).apply(
  ...
  def _data_generator(self):
    while True:
      while not self._replay_mem.ready_for_sample():
        time.sleep(5)
      for sample in self._replay_mem.sample_rollout():
