...
def _get_local_replays(replay_filelist):
  with open(replay_filelist, 'rt') as f:
    lines = f.readlines()
    tasks = [line.strip() for i, line in enumerate(lines)
             if i % hvd.size() == hvd.rank()]
  return tasks


class HvdImitationLearner(object):
  ...
  def __init__(self, ports, gpu_id, train_replay_filelist, val_replay_filelist,
    ...
    train_loss = tf.reduce_mean(model.il_loss * self.data_pool.train_batch_input.w)
    val_loss = tf.reduce_mean(model_val.il_loss * self.data_pool.val_batch_input.w)
    ...
    train_loss_aggregated = hvd.allreduce(train_loss)
    train_head_loss_aggregated = [hvd.allreduce(tf.reduce_mean(l * self.data_pool.train_batch_input.w))
                                  for l in model.losses]
    # train_loss = model.il_loss
    # train_head_losses = [tf.reduce_mean(l) for l in model.losses]
    val_loss_aggregated = hvd.allreduce(val_loss)
    val_head_losses_aggregated = [hvd.allreduce(tf.reduce_mean(l * self.data_pool.val_batch_input.w))
    ...
    self._checkpoints_dir = checkpoints_dir if hvd.rank() == 0 else None
    ...
    logger.configure(
        dir='training_log/rank{}'.format(hvd.rank()),
