...
from tleague.learners.piecewise_fusion import PiecewiseFusion
...
class HvdPPOLearner(PPOLearner):
  ...
  def __init__(self, league_mgr_addr, model_pool_addrs, learner_ports,
               ...
               distill_coef=1e-2, rollout_length=1, piecewise_fusion_schedule="-1", piecewise_fusion_schedule_vf="-1", **kwargs):
    ...
    trainer1 = tf.train.AdamOptimizer(learning_rate=self.LR, epsilon=1e-5)
    self.trainer = (tf.compat.v1.train.experimental.enable_mixed_precision_graph_rewrite(trainer1))
    burn_in_trainer1 = tf.train.AdamOptimizer(learning_rate=self.LR, epsilon=1e-5)
    self.burn_in_trainer = (tf.compat.v1.train.experimental.enable_mixed_precision_graph_rewrite(burn_in_trainer1))
    ...
    #PiecewiseFusion
    self._piecewise_fusion_schedule = piecewise_fusion_schedule  #"3;8"
    pieces = self._piecewise_fusion_schedule.split(';')
    boundaries = []
    for piece in pieces:
      boundaries.append(int(piece))

    self._piecewise_fusion_schedule_vf = piecewise_fusion_schedule_vf   #"3;8"
    pieces_vf = self._piecewise_fusion_schedule_vf.split(';')
    boundaries_vf = []
    for piece in pieces_vf:
      boundaries_vf.append(int(piece))
    if (len(boundaries) == 1) and (boundaries[0] <= 0):
      # without fusion
      gradvars = self.pw.non_fusion(grads, tvars)
      gradvars_vf = self.pw.non_fusion(grads_vf, tvars_vf)
    elif (len(boundaries) >= 1) and (boundaries[0] > 0):
      #PiecewiseFusion
      gradvars, _ = self.pw.piecewise_fusion(grads, tvars, boundaries, None, hvd.size(), self.batch_size, 'policy')
      gradvars_vf, _ = self.pw.piecewise_fusion(grads_vf, tvars_vf, boundaries_vf, None, hvd.size(), self.batch_size, 'value')
    else:
      raise ValueError('boundaries parameter is invalid')


    self._train_batch = self.trainer.apply_gradients(gradvars)
    self._burn_in = self.burn_in_trainer.apply_gradients(gradvars_vf)
