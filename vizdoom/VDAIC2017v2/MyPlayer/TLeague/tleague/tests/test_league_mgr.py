...
from tleague.base_league_mgr import ActorTask, LearnerTask
...
class TestLeagueManager(unittest.TestCase):
  ...
  def test_request_learner_task(self):
    league_client = LeagueManagerClient(league_mgr_addr="localhost:11007")
    task = league_client.request_learner_task(learner_id=str(uuid.uuid1()))
    self.assertTrue(isinstance(task, LearnerTask) or task is None)

  @timeout_decorator.timeout(2)
  def test_request_actor_task(self):
    league_client = LeagueManagerClient(league_mgr_addr="localhost:11007")
    task = league_client.request_actor_task(actor_id=str(uuid.uuid1()),
    ...
    self.assertEqual(task, None)
    learner_id = str(uuid.uuid1())
    league_client.request_learner_task(learner_id=learner_id)
    task = league_client.request_actor_task(actor_id=str(uuid.uuid1()),
                                            learner_id=learner_id)
    self.assertTrue(isinstance(task, ActorTask))

  @timeout_decorator.timeout(1)
  def test_notify_actor_task_begin(self):
    league_client = LeagueManagerClient(league_mgr_addr="localhost:11007")
    league_client.notify_actor_task_begin(actor_id=str(uuid.uuid1()))

  @timeout_decorator.timeout(1)
  def test_notify_actor_task_end(self):
    league_client = LeagueManagerClient(league_mgr_addr="localhost:11007")
    league_client.notify_actor_task_end(
        actor_id=str(uuid.uuid1()),
        match_result=MatchResult(str(uuid.uuid1()), str(uuid.uuid1()), 1))

  @timeout_decorator.timeout(1)
  def test_notify_learner_task_begin(self):
    league_client = LeagueManagerClient(league_mgr_addr="localhost:11007")
    league_client.notify_learner_task_begin(learner_id=str(uuid.uuid1()),
                                            learner_task=LearnerTask(model_key='1',
                                                                     hyperparam='2'))

  @timeout_decorator.timeout(1)
  def test_notify_learner_task_end(self):
    league_client = LeagueManagerClient(league_mgr_addr="localhost:11007")
    league_client.notify_learner_task_end(learner_id=str(uuid.uuid1()))

  @timeout_decorator.timeout(20)
  def test_checkpoint(self):
    league_client = LeagueManagerClient(league_mgr_addr="localhost:11007")
    model_client1 = ModelPoolClient(model_pool_addrs=["localhost:11001:11006"])
    hyperparam = MutableHyperparam()
    ...
    model_client2 = ModelPoolClient(model_pool_addrs=["localhost:11011:11016"])
    time.sleep(2)
