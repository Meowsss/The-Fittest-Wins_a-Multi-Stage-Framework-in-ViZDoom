...
class BoardMapObs(AppendObsInt):
    ...
    def reset(self, obs, **kwargs):
        ...
        self.wrapper = BoardMapObsFunc(obs, self.items, self.override, self.use_attr,
...
class AttrObsInt(AppendObsInt):
    class AttrFunc(object):
        ...
        def observation_transform(self, obs_pre, obs):
            ...
            return observation if self.override else list(obs_pre) + observation

    def reset(self, obs, **kwargs):
        super(AttrObsInt, self).reset(obs, **kwargs)
        self.wrapper = self.AttrFunc(obs, override=self.override,
                                     space_old=self.inter.observation_space)
...
class PosObsInt(AppendObsInt):
    ...
    def reset(self, obs, **kwargs):
        ...
        self.wrapper = self.PosFunc(obs, override=self.override,
                                     space_old=self.inter.observation_space)
...
class ActMaskObsInt(AppendObsInt):
    class ActMaskFunc(object):
        ...
        def observation_transform(self, obs_pre, obs):
            ...
            return list(obs_pre) + [act_mask.reshape([-1])]

    def reset(self, obs, **kwargs):
        super(ActMaskObsInt, self).reset(obs, **kwargs)
        self.wrapper = self.ActMaskFunc(obs, space_old=self.inter.observation_space)