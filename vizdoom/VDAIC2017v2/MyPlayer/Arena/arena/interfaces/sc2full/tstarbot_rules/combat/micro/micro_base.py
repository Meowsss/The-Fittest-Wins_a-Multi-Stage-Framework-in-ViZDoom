...
from ...data.pool.macro_def import COMBAT_ATTACK_UNITS
...
class MicroBase(object):
  ...
  def __init__(self):
    ...
    self.enemy_ground_combat_units = []
    self.enemy_ground_units = []
    self.enemy_air_combat_units = []
    self.enemy_air_units = []
  ...
  def update(self, dc):
    ...
    self.enemy_ground_combat_units = [e for e in self.enemy_combat_units
                                      if e.int_attr.unit_type not in AIR_UNITS]
    self.enemy_ground_units = [e for e in self.enemy_units
                               if e.int_attr.unit_type not in AIR_UNITS]
    self.enemy_air_combat_units = [e for e in self.enemy_combat_units
                                      if e.int_attr.unit_type in AIR_UNITS]
    self.enemy_air_units = [e for e in self.enemy_units
                               if e.int_attr.unit_type in AIR_UNITS]
  ...
  @staticmethod
  def find_nearest_n_units_to_pos(pos, units, n):
    base_x = pos['x']
    base_y = pos['y']
    dists = []
    for m in units:
