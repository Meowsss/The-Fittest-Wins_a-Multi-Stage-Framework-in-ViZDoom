...
def compute_area_dist(areas, timestep, pos, game_version):
  """ distance from area.ideal_base_pos to pos """
  pathing_grid = timestep.game_info.start_raw.pathing_grid
  ## what's the exactly version?
  if distutils.version.LooseVersion(game_version) >= distutils.version.LooseVersion('4.8'):
    array = (1 - bitmap2array(pathing_grid).transpose()) * 255
  else:
    array = bitmap2array(pathing_grid)[::-1].transpose()
  dist = {}
...
def get_slopes(timestep, game_version):
  ...
  if distutils.version.LooseVersion(game_version) >= distutils.version.LooseVersion('4.8.0'):
    pathing = (1 - bitmap2array(pathing_grid).transpose()) * 255
    placement = bitmap2array(placement_grid).transpose() * 255
    height = bitmap2array(terrain_height).transpose()
  else:
    pathing = bitmap2array(pathing_grid)[::-1].transpose()
    placement = bitmap2array(placement_grid)[::-1].transpose()
    height = bitmap2array(terrain_height)[::-1].transpose()
