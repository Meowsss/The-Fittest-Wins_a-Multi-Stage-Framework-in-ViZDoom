...
from .squad import SquadStatus, MutaliskSquadStatus
from .army import Army
from ..data.queue.combat_command_queue import CombatCmdType
from ..data.queue.combat_command_queue import CombatCommand
from ..data.pool.macro_def import COMBAT_FLYING_UNITS
from ..data.pool.macro_def import COMBAT_UNITS, COMBAT_UNITS_FOOD_DICT
from ..data.pool.macro_def import BUILDING_UNITS
from ..data.pool.map_tool import get_slopes
from .renderer import StrategyRenderer


Strategy = Enum('Strategy',
                ('RUSH', 'ECONOMY_FIRST', 'ONEWAVE', 'REFORM', 'REFORM_V2', 'HARASS'))
...
class ZergStrategyMgr(BaseStrategyMgr):
  def __init__(self, dc):
    ...
    self._food_trigger = _FOOD_TRIGGER
  ...
  def _init_config(self, dc):
    if hasattr(dc, 'config'):
      ...
      if hasattr(dc.config, 'combat_strategy'):
        ...
        elif dc.config.combat_strategy == 'REFORM_V2':
          self._strategy = Strategy.REFORM_V2
  ...
  def _command_army(self, cmd_queue):
    ...
    elif self._strategy == Strategy.REFORM_V2:
      self._organize_army_by_size(size=1)
      self._command_army_defend(cmd_queue)
      self._command_army_reform_v2(cmd_queue)
  ...
  def _command_army_reform_v2(self, cmd_queue):
    enemy_pool = self._dc.dd.enemy_pool
    ...
    attack_pos = enemy_pool.priority_pos
    if attack_pos is None:
      enemy_base_pos = {'x': self._dc.dd.base_pool.enemy_home_pos[0],
                        'y': self._dc.dd.base_pool.enemy_home_pos[1]}
      if enemy_pool.closest_cluster is None:
        attack_pos = enemy_base_pos
      else:
        if enemy_pool.closest_cluster.num_combat_units < 2:
          attack_pos = enemy_base_pos
        else:
          attack_pos = enemy_pool.closest_cluster.centroid
    ...
    if self._ready_to_attack:
      ...
      #   self._food_trigger = min(self._food_trigger, 195)
      #   return None
      if self._dc.sd.obs['player'][3] < (self._food_trigger/2):
        self._ready_to_attack = False
        self._ready_to_go = False
        return None
      for squad in self._army.squads:
