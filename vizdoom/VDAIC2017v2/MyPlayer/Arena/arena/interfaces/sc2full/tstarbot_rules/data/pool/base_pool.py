...
from .map_tool import compute_area_dist
...
class BasePool(PoolBase):
  def __init__(self, dd):
    ...
    self._dd = dd
    self.game_version = dd.game_version
    self._init = False
    self.home_pos = None
  ...
  def update(self, timestep):
    ...
    if not self._init:
      ...
      self.home_dist = compute_area_dist(self.resource_cluster, timestep,
                                         self.home_pos, self.game_version)
      d_min = 0
      for area in self.resource_cluster:
        ...
        if d > d_min:
          ...
          d_min = d
      self.enemy_home_dist = compute_area_dist(self.resource_cluster, timestep,
                                               self.enemy_home_pos, self.game_version)
      self._init = True
    else:
