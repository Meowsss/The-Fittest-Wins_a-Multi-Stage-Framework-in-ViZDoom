...
class PlayerEnv(gym.Env):
    ...
    def __init__(self, cfg, train_mode=1):
        self.cfg = cfg
        self.game = None
    ...
    def act(self, action):
        if not self.train_mode and action == self.all_actions[1]:
...
class VizdoomVecRwd(Wrapper):
    ...
    def reset(self, **kwargs):
        ...
        self.address_list = np.zeros((self.envs_num, 13))
    ...
    def rwd_transform(self, reward, game_var_pre, game_var, i):
        ...
        death_diff = game_var['DEATHCOUNT'] - game_var_pre['DEATHCOUNT']
        if death_diff != 0 or self.adress_sum[i] == 13:
            self.address_list[i] = np.zeros(13)
            binary_address[i] = np.zeros(13)
            self.adress_sum[i] = 0
        ...
        return (rwd_navi, rwd_kill_frag, rwd_killed_frag, rwd_hit, rwd_hitt, rwd_heal, rwd_shot1, rwd_shot2, rwd_armo, rwd_move)
...
def main():
    ...
    for episode in range(10):
        ...
        while True:
            ...
            if done:
                print('ep steps: {}; rwd: {}'.format(ep_step, rwd))
                break
            else:
                ep_step += 1
