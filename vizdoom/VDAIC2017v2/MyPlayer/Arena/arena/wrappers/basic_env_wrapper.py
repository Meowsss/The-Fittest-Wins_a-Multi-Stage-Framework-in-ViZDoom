...
class MergeUnits(ObservationWrapper):
  """ Wrapper for merging units info from two-players """

  def __init__(self, env):
    super(MergeUnits, self).__init__(env)
    # install observation space
    assert len(self.env.action_space.spaces) == 2

  def observation(self, timesteps):
