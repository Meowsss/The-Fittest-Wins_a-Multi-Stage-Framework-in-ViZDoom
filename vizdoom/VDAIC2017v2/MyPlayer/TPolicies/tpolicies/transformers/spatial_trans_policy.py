...
class SpatialTrans(BasePolicy):
  def __init__(self, ob_space=None, ac_space=None, sess=None,
    ...
    if input_data is None:
      ...
      if rl:
        ...
        A_loc, labels['s_select'], labels['m_select'] = input_data.A
      ...
      with tf.variable_scope("RL_value"):
        neglogp_final, entropy_final, vf = \
          self._make_rl_heads(ab_heads, sft_heads, noop_heads, tar_u_heads,
                              loc_heads, ss_heads, ms_heads, int_embed)
  ...
  def _make_il_loss(self, ab_heads, sft_heads, noop_heads,tar_u_heads,
                    loc_heads, ss_heads, ms_heads, labels):
  ...
  def _get_spa_embed(self, x_img):
    with tf.variable_scope('gridnet_enc'):
      with slim.arg_scope(my_vgg_arg_scope()):
        x = x_img  # NHWC
        x = slim.conv2d(x, 32, [5, 5], scope='conv1')  # [176, 200]
        x = slim.conv2d(x, 32, [3, 3], scope='conv2')  # [176, 200]
        x = slim.conv2d(x, 32, [3, 3], scope='conv3')  # [176, 200]
        x = slim.conv2d(x, 32, [3, 3], scope='conv4')  # [176, 200]
        enc_out = x
        xx_rates = []
        for rate in [3, 6, 12, 24]:  # from Yuan Gao
          xx = slim.conv2d(enc_out, 32, [3, 3], rate=rate,
                           scope=('rate%d_conv1' % rate))
          xx = slim.conv2d(xx, 32, [1, 1], scope=('rate%d_conv2' % rate))
          xx = slim.conv2d(xx, 4, [1, 1],
                           # activation_fn=None,
                           # normalizer_fn=None,
                           scope=('rate%d_out' % rate))
          xx_rates.append(xx)
        x = tf.add_n(xx_rates, name='added_out')
        spatial_embed = x

        x = slim.max_pool2d(x, [2, 2], scope='pool1')  # [88, 100]
        x = slim.conv2d(x, 16, [3, 3], scope='conv_small1')
        x = slim.max_pool2d(x, [2, 2], scope='pool2')  # [44, 50]
        x = slim.conv2d(x, 8, [3, 3], scope='conv_small2')
        x = slim.max_pool2d(x, [2, 2], scope='pool3')  # [22, 25]
        x = slim.conv2d(x, 4, [5, 5], scope='conv_small3')
  ...
  def _get_vec_embedding(self, x_vec):
    with tf.variable_scope("vec_enc"):
      x = slim.fully_connected(x_vec, 64, scope='x_fc1')
      x = slim.fully_connected(x, 64, scope='x_fc2')
    return x
  ...
  def _integrate_embed(self, spa_embed, trans_embed, vec_embed):
    ...
    with tf.variable_scope('inte_embed'):
      ...
      spa_embed = slim.conv2d(spa_embed, 32, [3, 3], scope='conv1')
      ...
      spa_embed = slim.fully_connected(spa_embed, 64, scope='fc1')
      ...
      trans_embed = slim.fully_connected(trans_embed, 64, scope='fc2')
  ...
  def _make_loc_head(self, spa_embed, ability_id_one_hot):

    self.prints.append(tf.print(tf.shape(spa_embed)))
