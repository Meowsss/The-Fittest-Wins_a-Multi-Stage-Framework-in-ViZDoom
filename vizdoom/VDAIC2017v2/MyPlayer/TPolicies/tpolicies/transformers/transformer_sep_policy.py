...
class TransformerSep(BasePolicy):
  ...
  def _create_net(self, x, masks, labels):
    ...
    with tf.variable_scope("ability"):
      x = slim.fully_connected(ori_x, self.enc_dim, scope='x_fc1')
      memory = self.encode(x, masks['len'], training=self.train)
    ...
    with tf.variable_scope("tar_unit"):
      ...
      tar_unit_logits, tar_unit_argmax, tar_unit_sam, tar_unit_neglogp = \
        self._make_ptr_head(query=ability_taken_f,
                            ...
                            memory=memory)
    # tar_loc_xy
    with tf.variable_scope("tar_loc_xy"):
      x = slim.fully_connected(ori_x, self.enc_dim, scope='x_fc1')
      memory = self.encode(x, masks['len'], training=self.train)
...
def transformer_test():
  ...
  sess = tf.Session()
  model = TransformerSep(sess, ob_space, ac_space)
  ...
  #   print(p)
  ps = sess.run(params)
  joblib.dump(ps, 'full_trans_sep.ckpt')
